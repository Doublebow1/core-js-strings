...
function countVowels(str) {
  ...
  return [...str].reduce(
    (num, character) => (vowels.includes(character) ? num + 1 : num),
    0
  );
}
...
function findLongestWord(sentence) {
  ...
  let longestSentence = sent[0];
  for (let i = 0; i < sent.length; i + 1) {
    if (sent[i].length > longestSentence.length) {
      longestSentence = sent[i];
    }
  }
...
function invertCase(/* str */) {
  ...
  //   .split('')
  //   .map((elem) => {
  //     if (elem === elem.toUpperCase() && elem !== elem.toLowerCase()) {
  //       return elem.toLowerCase();
  //     }
  //     if (elem === elem.toLowerCase() && elem !== elem.toUpperCase()) {
